...
public class ListPurchaseElements {
	...
	@FindBy(xpath = "//*[@id=\"purchase_table\"]/thead/tr[i]/th[2]") // Referenece column.Value of i changes in the for
																		// loop.
	...
	public boolean purchasefilter(List<WebElement> element, String comparestring) {
		for (int i = 1; i <= ((List<WebElement>) element).size(); i++) {
			...
			if (searchstring.contains(comparestring)) {
				System.out.println("Purchase page -Filter Functionality Working" +element);
			}
		}
		return true;
	}
	public boolean searchPurhase(String searchPurhase) throws InterruptedException {
		for (int i = 0; i < referencenocolumn.size(); i++) {
			String refno = referencenocolumn.get(i).getText();
			if (searchPurhase.equals(refno)) {
				System.out.println("Purhase Found");
				break;
			}
		}
		return true;
